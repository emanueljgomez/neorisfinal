...
export default class EditShift extends Component {
  ...
  componentDidMount() {
    axios
      .get("http://localhost:5000/shifts/" + this.props.match.params.id)
      .then((response) => {
        this.setState({
  ...
  onSubmit(e) {
    ...
    if (shift.duration === "") {
      ...
      axios
        .post(
          "http://localhost:5000/shifts/update/" + this.props.match.params.id,
          shift
        )
  ...
  render() {
    return (
      <div style={{ width: 30 + "rem" }}>
        <h3 style={{ margin: 1 + "vw" }}>Editando Turno Laboral</h3>
